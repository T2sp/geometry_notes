...
\begin{figure}[H]
    \centering
    \begin{tikzcd}[row sep=large, column sep=large]
        M \ar[d, "g"']\ar[r, hook, "f"] &N \ar[dl, red, dashed, "\exists h"] \\
        I &
    \end{tikzcd}
    \caption{単射的加群}
    \label{fig:inj-mod}
...
\begin{mydef}[label=def:double-complex]{二重複体}
    ...
    \begin{align}
        ...
        d_2^{p+1,\, q} \circ d_1^{p,\, q} + d_1^{p,\, q+1} \circ d_2^{p,\, q} &= 0
    \end{align}
...
二重複体の射は図式の射として定義する．この射によって $\MOD{R}$ 上の二重複体全体は圏を成すので，これを $\mathrm{C}_2 (\MOD{R})$ と書く．$\mathrm{C}_2 (\MOD{R})$ はアーベル圏をなす．
...
ここまでの準備の下，命題\ref{def:LDF-basic}を示す．
