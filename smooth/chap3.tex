...
与えられたベクトル空間 $V,\, W,\, Z \in \Obj{\VEC{\mathbb{K}}}$ に対して，集合
\begin{align}
	\bm{L(V,\, W;\, Z)} \coloneqq \bigl\{\, F \colon V \times W \lto Z \bigm| F\;\text{は双線型写像} \,\bigr\} 
\end{align}
を考える．$L(V,\, W;\, Z)$ の上の加法とスカラー乗法を $\forall v \in V,\, \forall w \in W,\, \forall \lambda \in \mathbb{K}$ に対して
\begin{align}
	(F + G)(v,\, w) &\coloneqq F(v,\, w) + G(v,\, w), \\
	(\lambda F)(v,\, w) &\coloneqq \lambda \bigl(F(v,\, w)\bigr)
\end{align}
によって定義すると $L(V,\, W)$ は $\mathbb{K}$ ベクトル空間になる．
そして $\forall \omega \in V^*,\; \forall \eta \in W^*$ に対して，$\bm{\omega \otimes \eta}$ と書かれる $L(V,\, W;\, \mathbb{K})$ の元を
\begin{align}
	\bm{\omega \otimes \eta} \colon V \times W \lto \mathbb{K},\; (v,\, w) \lmto \omega(v)\eta(w)
\end{align}
によって定義する（つまり，$\omega \otimes \eta$ は双線型写像である）．
...
\begin{proof}
	...
	\begin{align}
		\textcolor{red}{\overline{\Phi}}(\varepsilon_\mu \otimes \eta_\nu )(v,\, w) = \varepsilon_\mu (v) \eta_\nu (w) = \varepsilon^\mu \otimes \eta^\nu(v,\, w)
	\end{align}
	が成り立つ（ただし，左辺の $\otimes$ は命題\ref{prop:tensor-vec}，右辺は命題\ref{prop:tensor-multillinear}で定義したものである），ので
	$\textcolor{red}{\overline{\Phi}}$ は $\mathcal{E}$ の元と $\mathcal{B}$ の元の1対1対応を与えるので同型写像である．
\end{proof}
